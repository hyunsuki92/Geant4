...
fDetectorRadius = 4*cm;
fDetectorLength= 5*cm;
G4Material* NaI = G4NistManager::Instance()->FindOrBuildMaterial("G4_SODIUM_IODIDE"); 

G4Tubs* solidDetector = new G4Tubs("Detector",0.,fDetectorRadius,fDetectorLength,0.,360.*deg);
logicDetector = new G4LogicalVolume(solidDetector,NaI,"Detector");
G4VPhysicalVolume* physDetector =new G4PVPlacement(0,G4ThreeVector(0.,0.,5.*cm),logicDetector,"Detector",logicWorld,false,0);
