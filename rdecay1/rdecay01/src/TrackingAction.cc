...
void TrackingAction::PreUserTrackingAction(const G4Track* track)
{
  ...
  //which volume?
  G4VPhysicalVolume* pVolume = track->GetVolume();
  G4int iVol = 0;
  ...
  if (ih) G4AnalysisManager::Instance()->FillH1(ih, Ekin); do not fill histograms
 */ 
  //gamma
/*
 if (particle == G4Gamma::Gamma()) {
analysis->FillNtupleIColumn(0,0, pid);
analysis->FillNtupleIColumn(0,1, Z);
analysis->FillNtupleIColumn(0,2, A);
analysis->FillNtupleDColumn(0,3, Ekin);
analysis->FillNtupleDColumn(0,4, time/s);
analysis->FillNtupleDColumn(0,5, weight);
